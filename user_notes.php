	...
	<form method="post">
		<div class="col">
			<div class=formulaire>
				...
				<div>
					<label for="price">Trier par notes globales (Meilleure à la moins bonne) : </label>
                    <input type="submit" name="desc_rate_global" id = "desc_rate_global" value="Go">
				</div>

				<div>
					<label for="price">Trier par notes globales (Moins bonne à la meilleure) : </label>
                    <input type="submit" name="asc_rate_global" id = "asc_rate_global" value="Go">
				</div>
  					<br>
			</div>
		</div>
...
<table>
		...
		$sql3 = 'SELECT DISTINCT product.name as "nom", rating.dateOfPub as "date",utilisateur.username as "user", utilisateur.image as image, rating.rate as "rate",rating.comm as "comm" 
		FROM utilisateur,rating,product WHERE product.id = rating.idProduct AND utilisateur.id = rating.idUser AND utilisateur.username ="'.$test.'"'.' ORDER BY (SELECT rating.dateOfPub FROM rating)
		 DESC';
		
		 $sql4 = 'SELECT DISTINCT product.name as "nom", rating.dateOfPub as "date",utilisateur.username as "user", utilisateur.image as image, rating.rate as "rate",rating.comm as "comm" 
		 FROM utilisateur,rating,product WHERE product.id = rating.idProduct AND utilisateur.id = rating.idUser AND utilisateur.username ="'.$test.'"'.' ORDER BY (SELECT rating.dateOfPub FROM rating)
		  ASC';
		...
		} else if (isset($_POST['desc_rate_global'])) {
		...
		} else if (isset($_POST['asc_rate_global'])) {
